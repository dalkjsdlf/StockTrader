...
public class OrderController {
    ...
    public ResponseEntity<BuyOrderResponse> placeBuyOrder(@Valid @RequestBody BuyOrderRequest buyOrderRequest) {
        final Order order = buyOrderRequest.toDomain(); // DTO가 Domain으로 변환
        final Order savedOrder = orderService.buyStock(order);
        final BuyOrderResponse response = BuyOrderResponse.from(savedOrder); // Domain을 DTO로 변환
